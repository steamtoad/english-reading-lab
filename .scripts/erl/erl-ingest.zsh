...
# erl-ingest
# Тип: ERL CLI
# Назначение: загрузить один JSON-объект в canonical ERL storage
#------------------------------------------------------------------------------
...
usage() {
    cat <<'EOF'
Usage:
  erl-ingest --input FILE
  erl-ingest --input -
  erl-ingest --input FILE --id ID

Options:
  --input FILE    JSON object; "-" means stdin
  --id ID         override .id/.uuid from the document
  -h, --help      show help
EOF
}
...
if [[ "$input" == "-" ]]; then
    tmp_input="$(mktemp "${TMPDIR:-/tmp}/erl-ingest.XXXXXX")"
    cat > "$tmp_input"
    input="$tmp_input"
fi
...
[[ -n "$id" ]] \
    || die "document has neither .id nor .uuid; use --id"
...
destination="${RECORDS_DIR}/${id}.json"
tmp_destination="$(mktemp "${destination}.tmp.XXXXXX")"
backup_destination=""

if ! jq -S '.' "$input" > "$tmp_destination"; then
    rm -f -- "$tmp_destination"
    die "cannot canonicalize JSON"
fi
...
if ! "$ERL_CHECK" >&2; then
    ...
    print -u2 -- "erl-ingest: erl-check failed; mutation rolled back:"
